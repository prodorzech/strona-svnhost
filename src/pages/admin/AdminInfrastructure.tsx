          ...
          <div className="dash-card" style={{ padding: 20, marginBottom: 16, background: 'linear-gradient(135deg, rgba(34,197,94,0.08), rgba(16,185,129,0.05))', borderColor: 'rgba(34,197,94,0.2)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>
                  Panel SVNHost używa SQLite (better-sqlite3) jako bazy danych. Plik: <code>backend/data/svnhost.db</code>. Tabele: <code>users</code>, <code>sessions</code>, <code>servers</code>. Hasła: bcrypt. Sesje: UUID token. Domyślny admin: <code>admin@svnhost.pl</code> / <code>admin123</code>.
                </p>
              </div>
            </div>
          </div>

          <GuideSection id="db-info" title="📋 Architektura — co gdzie jest" icon={<Info size={16} style={{ color: '#6366f1' }} />}>
            <CodeBlock lang="text" code={`backend/
├── data/svnhost.db              ← plik SQLite (auto-tworzony)
├── src/database.ts              ← initDatabase(), CRUD users/sessions
├── src/authMiddleware.ts        ← authMiddleware, adminMiddleware
├── src/authRoutes.ts            ← /register, /login, /me, /logout, admin CRUD
└── src/index.ts                 ← app.use('/api/auth', authRouter)

src/services/backendApi.ts       ← backendApi.auth.*, token w localStorage
src/store/store.ts               ← loadCurrentUser(), setCurrentUserFromApi()
src/pages/auth/Login.tsx         ← async login via backendApi
src/pages/auth/Register.tsx      ← async register via backendApi
src/App.tsx                      ← loadCurrentUser() on startup
...
Endpointy API (/api/auth/):
  ...
  POST /logout          (Bearer token)                 → wyloguj sesję
  POST /logout-all      (Bearer token)                 → wyloguj wszystko
  PUT  /profile         { username?, fullName?, ... }   → { user }
  POST /change-password { currentPassword, newPassword }→ { token }
  GET  /sessions        (Bearer token)                 → lista sesji
  GET  /admin/users     (admin only)                   → lista userów
  PUT  /admin/users/:id (admin only)                   → edytuj usera
  DELETE /admin/users/:id (admin only)                 → usuń usera`} />
          </GuideSection>
...
echo 'Gotowe — zależnosci auth zainstalowane'`} />
          ...
          <GuideSection id="db-cmd2" title="⚡ Komenda 2 — Test wszystkich endpointów" icon={<Key size={16} style={{ color: '#8b5cf6' }} />}>
            <p>Skopiuj i wklej — przetestuje rejestrację, login, sesję, admin i wylogowanie:</p>
            <CodeBlock lang="bash" code={`# ══════════════════════════════════════════════════════
#  Test auth API (zamien localhost:3001 na swoj adres)
# ══════════════════════════════════════════════════════
...
curl -s -X POST $API/register \\
  ...
  -d '{"email":"test@test.pl","username":"TestUser","password":"test123"}' && \\

echo "" && echo "=== 2. Login admin ===" && \\
TOKEN=$(curl -s -X POST $API/login \\
  -H "Content-Type: application/json" \\
  -d '{"email":"admin@svnhost.pl","password":"admin123"}' | grep -o '"token":"[^"]*"' | cut -d'"' -f4) && \\
echo "Token: $TOKEN" && \\

echo "=== 3. Sprawdz sesje ===" && \\
curl -s $API/me -H "Authorization: Bearer $TOKEN" && \\

echo "" && echo "=== 4. Lista userow (admin) ===" && \\
curl -s $API/admin/users -H "Authorization: Bearer $TOKEN" && \\

echo "" && echo "=== 5. Wylogowanie ===" && \\
curl -s -X POST $API/logout -H "Authorization: Bearer $TOKEN" && \\

echo "" && echo 'Wszystkie endpointy OK'`} />
          </GuideSection>

          <GuideSection id="db-cmd3" title="⚡ Komenda 3 — Backup + bezpieczeństwo" icon={<AlertTriangle size={16} style={{ color: '#ef4444' }} />}>
            <p>Backup bazy, uprawnienia pliku + automatyczny cron:</p>
            <CodeBlock lang="bash" code={`# ══════════════════════════════════════════════════════
...
# Backup teraz
cp "$DB" ~/backups/svnhost-$(date +%Y%m%d-%H%M).db && \\

# Uprawnienia — tylko owner moze czytac
chmod 600 "$DB" && \\
...
echo 'Backup OK, chmod 600, cron ustawiony' && \\
echo 'Backupy w: ~/backups/' && \\
ls -la ~/backups/`} />
            <div style={{ marginTop: 14, padding: 14, background: 'rgba(239,68,68,0.06)', borderRadius: 8, border: '1px solid rgba(239,68,68,0.15)' }}>
              <p style={{ fontWeight: 600, color: '#ef4444', marginBottom: 4 }}>⚠️ Nie zapomnij</p>
              <p>1. <strong>Zmien haslo admina</strong> po pierwszym logowaniu (admin123 → coś silnego)<br/>
              2. <strong>Podglad bazy z terminala:</strong> <code>sqlite3 ~/strona-svnhost/backend/data/svnhost.db</code> → <code>SELECT * FROM users;</code></p>
            </div>
          </GuideSection>
