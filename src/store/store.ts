...
export function purchaseServer(userId: string, planId: string, serverName: string, options?: { botRuntime?: 'nodejs' | 'python' }): { success: boolean; error?: string; serverId?: string } {
  ...
  const server: Server = {
    ...
    sshPort: 22,
    sshUser: (plan.type === 'vps') ? 'root' : 'svnhost',
    sshPassword: Math.random().toString(36).slice(2, 10) + Math.random().toString(36).slice(2, 6).toUpperCase(),
